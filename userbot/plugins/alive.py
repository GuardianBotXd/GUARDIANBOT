...
from userbot import StartTime, guardian, guardianversion

from ..Config import Config
from ..core.managers import eor
from ..helpers.functions import check_data_base_heal_th, get_readable_time, guardianalive
...
@guardian.guardian_cmd(
    pattern="guardian$",
    command=("guardian", menu_category),
    info={
...
async def amireallyalive(event):
    ...
    start = datetime.now()
    guardianevent = await eor(event, "`Checking...`")
    end = datetime.now()
    ...
    try:
        ...
        return await eor(
            guardianevent,
            f"**Media Value Error!!**\n__Change the link by __`.setdv`\n\n**__Can't get media from this link :-**__ `{GUARDIAN_IMG}`",
        )
...
**{EMOTES} Guardianuserbot Version :** `{guardianver}`
...
async def amireallyalive(event):
    ...
    guardian_caption = "**GuardianBot Is Online**\n\n"
    guardian_caption += f"**{EMOJI} Telethon version :** `{version.__version__}\n`"
    guardian_caption += f"**{EMOJI} Guardianuserbot Version :** `{guardianversion}`\n"
    guardian_caption += f"**{EMOJI} Python Version :** `{python_version()}\n`"
    guardian_caption += f"**{EMOJI} Uptime :** {uptime}\n"
    guardian_caption += f"**{EMOJI} Master:** {mention}\n"
    results = await event.client.inline_query(Config.BOT_USERNAME, guardian_caption)
    await results[0].click(event.chat_id, reply_to=reply_to_id, hide_via=True)
    await event.delete()
