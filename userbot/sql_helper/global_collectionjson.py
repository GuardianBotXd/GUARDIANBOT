...
class Guardian_GlobalCollection_Json(BASE):
    __tablename__ = "guardian_globalcollectionjson"
...
Guardian_GlobalCollection_Json.__table__.create(checkfirst=True)
...
def add_collection(keywoard, json, njson=None):
    ...
    if to_check:
        keyword_items = SESSION.query(Guardian_GlobalCollection_Json).get(keywoard)
        SESSION.delete(keyword_items)
    keyword_items = Guardian_GlobalCollection_Json(keywoard, json, njson)
...
def get_collections():
    try:
        return SESSION.query(Guardian_GlobalCollection_Json).all()
    finally:
        SESSION.close()
